...
namespace Mongo.Services.ShoppingCart.Controllers;
...
[HttpPost("CartUpsert")]
    ...
    public async Task<ResponseDto> RemoveCart([FromBody] int cartDetailsId)
    {
        try
        {
            ...
            int totalCountCartItem = _db.CartDetails.Count(u => u.CartHeaderId == cartDetails.CartHeaderId);
            _db.CartDetails.Remove(cartDetails);
            if (totalCountCartItem == 1)
            {
                var cartHeaderToRemove =
                    await _db.CartHeaders.FirstOrDefaultAsync(
